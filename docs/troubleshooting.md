...
- Gemma 4 E4B is killed by iOS with signal 9 on the first image prompt after clearing the GPU cache: the cold-cache Metal serialization path can exceed the device memory high-water mark before the app reaches image resize or the release-before-prefill hook. Current sample app defaults set `kLiteRtLmAdvancedCacheCompiledShadersOnly` on both the main and vision GPU executors, which keeps the first-run path inside the tested iPhone 17 Pro Max memory budget. If you override `LITERT_LM_GPU_CACHE_COMPILED_SHADERS_ONLY=false` or `LITERT_LM_VISION_GPU_CACHE_COMPILED_SHADERS_ONLY=false`, expect the older cold-cache behavior.
