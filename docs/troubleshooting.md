...
- **`GPU sampler unavailable. Falling back to CPU sampling.`**: current package artifacts bundle Google's iOS TopK Metal sampler as `LTMTS.framework`, and the LiteRT-LM patch falls back from `libLiteRtTopKMetalSampler.dylib` to `@rpath/LTMTS.framework/LTMTS`. Treat this warning as a regression in current builds. Check that the app embeds `LTMTS.framework`, that the runtime log shows `Configured LiteRT runtime library directory=.../Frameworks`, and that `LRE.framework/LRE` exports the sampler's required LiteRT ABI symbols (`LiteRtCreateModelFromFd` and `LiteRtGetBlockWiseQuantization`).
- **Objective-C "Class ... is implemented in both ... LTMTS.framework ... and ... LMA.framework" warnings on iOS startup**: this has been observed with Google's official TopK Metal sampler and Metal accelerator prebuilts. Both binaries statically contain the same private Objective-C logging/runtime helper classes. The warning did not prevent the physical iPhone Gemma 4 image regression from passing, but keep it visible when updating upstream sampler/accelerator prebuilts because it has to be fixed in the upstream binary packaging rather than in Swift code.
