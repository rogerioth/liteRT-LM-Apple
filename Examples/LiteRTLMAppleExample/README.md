...
- exercise the same flow on iPhone, iPad, Apple TV, Apple Vision Pro, or Mac
...
On `feat/tvos-support`, the project is configured to resolve `LiteRTLMApple` from the matching GitHub branch through Swift Package Manager so the sample reflects the in-progress package state, including the new tvOS slices.

If you are running on a physical iOS device, choose your signing team in Xcode before building.

For tvOS validation, choose `Apple TV`, `Any tvOS Device`, or another Apple TV simulator destination in Xcode. The checked-in tvOS device and simulator slices are Apple Silicon only and are currently derived from the packaged iOS device and iOS simulator dylibs because upstream does not ship dedicated tvOS binaries.
...
The current checked-in iOS simulator, tvOS simulator, visionOS simulator, and Catalyst artifacts are `arm64` only, so Intel simulator or Intel Catalyst builds are not supported by this repository as-is.
...
- a dedicated Apple TV SwiftUI interface tuned for focus and large-screen navigation
